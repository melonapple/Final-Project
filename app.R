...
load('data/Project.RData', verbose=T)
...
trade_plotting=function(x, y, type, combine=F, color='black'){
  
  x <- enquo(x)
  y <- enquo(y)
  
  if (combine==F){
    trade %>% filter(품목==type) %>% ggplot(mapping=aes(x=!!x, y=!!y)) +
    ...
    trade %>% ggplot(mapping=aes(x=!!x, y=!!y, color=품목)) + 
      geom_line() +
      geom_point() + 
      scale_x_continuous(breaks=2010:2020)
...
year_location_spatialdata=function(year=2010, type=누적영업, limit=18000){
  
  type=enquo(type)
  
  
  result = cafe_year_location_spatial %>% filter(년도==year) %>% ggplot() + 
    geom_polygon(aes(x = long,
                     y=lat,
                     group = group,
                     fill = !! type),
                 color='black') + 
    scale_fill_gradient(low='white', high='brown', limits = c(0, limit))
...
seoul_year_location_spatialdata=function(year=2010, type=누적영업, limit=7000){
  
  type=enquo(type)
  
  
  result = seoul_cafe_year_location_spatial %>% filter(년도==year) %>% ggplot() + 
    geom_polygon(aes(x = long,
                     y=lat,
                     group = group,
                     fill = !! type),
  ...
  fluidPage(
    ...
    sidebarLayout(
      ...
      sidebarPanel(
        ...
        numericInput('year', label='년도(2010 - 2020)', 
                  value=2020,
                  )
